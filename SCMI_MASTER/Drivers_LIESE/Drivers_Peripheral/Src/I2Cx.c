...
bool I2Cx_GPIO_Init(bool PUR, uint8_t x){
	if(x==1){//I2C1: PB6 -> SCL, PB7 -> SDA
		...
		GPIOx_InitAF(GPIOB, 6, GPIO_OTYPER_OD, GPIO_OSPEEDR_HS,GPIO_AFR_AFSEL_I2C);//Pin 6
		GPIOx_InitAF(GPIOB, 7, GPIO_OTYPER_OD, GPIO_OSPEEDR_HS,GPIO_AFR_AFSEL_I2C);//Pin 7
		if(PUR){
		...
		GPIOx_InitAF(GPIOH, 4, GPIO_OTYPER_OD, GPIO_OSPEEDR_HS,GPIO_AFR_AFSEL_I2C);//Pin 6
		GPIOx_InitAF(GPIOH, 5, GPIO_OTYPER_OD, GPIO_OSPEEDR_HS,GPIO_AFR_AFSEL_I2C);//Pin 7
		if(PUR){
		...
		RCC_AHB1ENR |= RCC_AHB1ENR_GPIOHEN ;//Habilitar Fuente de reloj H antes de configurar
		GPIOx_InitAF(GPIOH, 7, GPIO_OTYPER_OD, GPIO_OSPEEDR_HS,GPIO_AFR_AFSEL_I2C);//Pin 6
		GPIOx_InitAF(GPIOH, 8, GPIO_OTYPER_OD, GPIO_OSPEEDR_HS,GPIO_AFR_AFSEL_I2C);//Pin 7
		if(PUR){
...
void I2Cx_EV_CallBack(uint8_t x, I2C_HandlerDef * i2c){
	...
	if(I2Cx_GetFlagSR2(I2C_SR2_MSL, i2c)){//Master mode
		if(I2Cx_GetFlagSR1(I2C_SR1_SB, i2c)){//SB set
			...
			//RXNE=1, BTF=0
			if ((I2Cx_GetFlagSR1(I2C_SR1_RXNE, i2c))&&(!I2Cx_GetFlagSR1(I2C_SR1_BTF, i2c))){
				if((i2c->sizeI2C)>3){
